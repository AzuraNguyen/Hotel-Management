...
class KhachHang(models.Model):
    ...
    loaiKhach = models.CharField(
        choices=LOAIKHACH_CHOICES, default=NOIDIA, max_length=2)
    gioiTinh = models.CharField(
        max_length=1, choices=GIOITINH_CHOICES, default=NAM)
    cccd = models.CharField(max_length=12, null=False)
    quocTich = models.CharField(
        max_length=20, choices=QUOCTICH_CHOICES, default=VIETNAM)
    ...
    class Meta:
        constraints = [
            CheckConstraint(check=Q(hoTen__length__gt=0),
                            name="KH_hoTen_minlength"),
            CheckConstraint(check=Q(cccd__length__gt=0),
                            name="KH_cccd_minlength"),
            CheckConstraint(check=Q(quocTich__length__gt=0),
                            name="KH_quocTich_minlength"),
        ]
...
class LoaiPhong(models.Model):
    ...
    class Meta:
        constraints = [
            ...
            CheckConstraint(check=Q(slNguoi__gt=0),
                            name='LP_check_so_luong_nguoi'),
        ]
...
class Phong(models.Model):
    ...
    trangThai = models.IntegerField(
        choices=TRANGTHAI_CHOICES, default=TRONG, null=False)
...
class PhieuThuePhong(models.Model):
    ...
    trangThai = models.IntegerField(
        null=False, choices=TRANGTHAI_CHOICES, default=CHUATHANHTOAN)
...
class User(models.Model):
    ...
    chucVu = models.IntegerField(
        choices=CHUCVU_CHOICES, default=NHANVIEN, null=False)
    ...
    class Meta:
        constraints = [
            CheckConstraint(check=Q(hoTen__length__gt=0),
                            name='USER_check_hoTen'),
            CheckConstraint(check=~Q(gioiTinh__length=0),
                            name='USER_check_gioiTinh'),
        ]
...
class DangNhap(models.Model):
    ...
    class Meta:
        constraints = [
            CheckConstraint(check=Q(tenDN__length__gt=0),
                            name='DN_check_tenDN'),
            CheckConstraint(check=Q(password__length__gt=0),
...
class HoaDon(models.Model):
    ...
    id_PTP = models.ForeignKey(
        PhieuThuePhong, null=False, on_delete=models.CASCADE)
    ...
    class Meta:
        constraints = [
            CheckConstraint(check=Q(donGiaPhong__gte=0),
                            name='HD_check_donGiaPhong'),
            CheckConstraint(check=Q(tienDichVu__gte=0),
                            name='HD_check_tienDichVu')
        ]
...
class ChiTietHoaDon(models.Model):
    ...
    class Meta:
        constraints = [
            ...
            UniqueConstraint(fields=['id_HD', 'id_DV'],
                             name='CTHD_unique_HD_DV'),
        ]
